...
// GET /api/attachments?post_id=xxx — list attachments for a post
export async function GET(request) {
  try {
    ...
    if (postId) {
      ...
      sql = `
        SELECT a.*, p.title as post_title, p.slug as post_slug, p.category_id as post_category_id, p.category_name as post_category_name
...
// POST /api/attachments — upload attachment (base64 or URL)
      ...
      if (matches) {
        ...
        if (!uploaded) {
          try {
            ...
            console.warn('R2/Local upload failed for attachment, keeping base64:', fsErr.message);
          }
        }
      }
    }
