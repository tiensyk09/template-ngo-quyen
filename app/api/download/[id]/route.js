...
export async function GET(request, { params }) {
  try {
    ...
    // Xác thực token tải tệp tạm thời (thời hạn 5 phút) để chống DDoS/Scraping
    ...
    // Dọn dẹp các token đã hết hạn trong CSDL
    try {
      await query('DELETE FROM download_tokens WHERE expires_at < ?', [Date.now()]);
    } catch (e) {}

    // Kiểm tra giới hạn tải tối đa 3 lần cho token này
    ...
    // Stream tệp tin trực tiếp thay vì 302 redirect để ẩn đường dẫn thật (ngăn chặn DDoS/Scraping)
    
    // 1. Xử lý tệp tin lưu cục bộ (Local filesystem)
    ...
    // 2. Xử lý tệp tin trên Cloudflare R2
    try {
      ...
      // Không chạy trên môi trường Cloudflare (Local dev)
    }

    // 3. Dự phòng: Fetch và stream từ đường dẫn từ xa (Remote URL proxy)
    try {
